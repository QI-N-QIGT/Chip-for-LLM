...
随着大语言模型（LLM）的飞速发展，其惊人的能力背后是对计算和存储资源的巨大需求。如何设计高效的芯片与系统来加速 LLM 的训练与推理，已成为学术界和工业界共同关注的焦点。由于芯片community利用开源平台较少，所以从芯片视角整合相关资源的repo几乎没有找到。因此我创建了一个系统性的资源整合平台。

当前AI芯片设计，跨越不同领域进行合作变得十分的关键。算法-芯片-软件-封装-热设计需要进行全栈的协同设计。


本项目的创立初衷，正是为了系统性地梳理和介绍 **“面向大语言模型的芯片设计与加速技术”** 的算法，芯片与系统的部分知识和最新进展，并且，将一些好的repo,博客，论文等展示出来，为AI chip后续的科研，工程优化提供便利。

我们希望将以下核心内容整合在一起：

1. **LLM算法**：介绍当前主流 LLM 的网络结构与新兴趋势。
2. **硬件需求**：详细分析 LLM 对芯片在计算、内存、功耗等方面的核心需求。
3. **加速方案**：系统梳理学术界的前沿路径与工业界的最佳实践。
4. **系统技术**：探讨芯片之上的互联、软件栈等关键系统级技术。
5. **优质资源**：汇集领域内高质量的论文、开源代码和基准测试，构建一个 "Awesome List"。

希望我能把挖的坑尽快的填完，也希望有同道中人一同填坑。

整个项目的目录暂时如下（09/27/2025）

## **目录 (Table of Contents)**

* 1. LLM 算法 (LLM Fundamentals)

  * 1.1 Transformer 核心架构
  * 1.2 主流 LLM 架构演进历程  ----------------- v1
  * 1.3 新兴 LLM 架构（2025年以来的算法架构创新，创新点）
...
* **FlashAttention: Fast and Memory-Efficient Exact Attention with IO-Awareness** \- *NeurIPS 2022*通过 IO 感知的注意力算法设计，显著减少了 GPU HBM 读写量，成为 Transformer 加速的里程碑式工作。

### **5.3 专用加速器 (ASIC/FPGA)**

* **In-Datacenter Performance Analysis of a Tensor Processing Unit** \- *ISCA 2017*Google TPU v1 的开创性论文，详细介绍了其设计哲学和在数据中心的实际性能。
